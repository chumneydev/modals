...
$modules->get('PageQueryBoss')->isSingular = true;
$modules->get('PageQueryBoss')->isAutoload = false;
...
// create new pages from json feed
function createNewPageFromFeed($feed, $feedID, $crc32) {
    $p = new Page();
    $p->setOutputFormatting(false);
    $p->template = wire('templates')->get("feed-item");
    $p->parent = wire('pages')->get("/development/");
    $p->name = $feed->title . $feed->id;
    $p->title = $feed->title;
    //$p->status = $feed->status
    $p->body = $feed->body;
    $p->crc32 = $crc32;
    $p->feed_id = $feedID;
    $p->save();
...
// update pages from json feed if page already exists
function updatePageFromFeed($p, $feed, $crc32) {
    $p->of(false);
    $p->title = $feed->title;
    //$p->status = $feed->status;
    $p->body = $feed->body;
